...
const addRefundApply = (params,callback) => {
    let query = " insert into refund_apply(order_id,payment_id,date_id,apply_reason,apply_fee";
    if (params.adminId){
        query += ",admin_id"
    }
    if (params.userId){
...
const getRefundApply = (params,callback) => {
    let query = " select ra.*,uo.created_type,pi.payment_type,au.real_name,pi.total_fee,pi.bank,pi.bank_code,pi.account_name,uo.route_start,uo.route_end  from refund_apply ra" +
...
const updateRefundByOrder = (params,callback) => {
    let paramsArray = [],i=0;
    let query = " update refund_apply set payment_refund_id = ? , status = ? where order_id = ? and payment_id = ?";
    paramsArray[i++] = params.paymentRefundId;
    paramsArray[i++] = params.status;
    paramsArray[i++] = params.orderId;
    paramsArray[i] = params.paymentId;
    db.dbQuery(query,paramsArray,(error,rows)=>{
        logger.debug('updateRefundByOrder');
        callback(error,rows);
    })
}
module.exports = {
